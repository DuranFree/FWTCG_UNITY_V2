Shader "UI/CardShine"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Radial gradient centered on (_ShineX, _ShineY)
                float2 shineCenter = float2(_ShineX, _ShineY);
                float dist = distance(IN.texcoord, shineCenter);

                // Smooth falloff from center
                float shine = saturate(1.0 - dist / _ShineRadius);
