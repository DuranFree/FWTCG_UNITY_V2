...
### 游戏状态
- [ ] G 全局状态对象（所有字段 1:1 移植为 C# GameState 类）
- [ ] 回合系统（awaken → start → summon → draw → action → end 六阶段）
- [ ] 回合倒计时（30秒，时间到自动结束玩家回合）
- [ ] 胜利判定（率先8分 / 传奇HP归零 / 平局）
- [ ] 得分系统（据守+1 / 征服+1 / 燃尽惩罚+1）
- [ ] 最后1分受限规则（得第8分须本回合主动征服所有战场）
...
### 回合流程
- [ ] 唤醒阶段（解除休眠、重置符文、清除眩晕）
- [ ] 开始阶段（据守得分判定 + 战场特殊能力触发）
- [ ] 召出阶段（从符文牌堆召出2张，后手首回合3张）
- [ ] 抽牌阶段（抽1张 + 燃尽检查 + 法力/符能清零）
- [ ] 行动阶段（玩家操作 / AI 操作）
- [ ] 结束阶段（眩晕清除 / 标记伤害清除 / 临时效果清除 / 时间扭曲检查）

### 单位状态
- [ ] atk = HP 规则（currentHp 和 currentAtk 始终同步）
- [ ] 增益指示物系统（buffToken，+1/+1，不可叠加）
- [ ] 休眠（exhausted）/ 眩晕（stunned）状态
...
- [ ] 溢出伤害检查（压制关键词击中传奇）
- [ ] 战斗结果判定（双方全灭 / 进攻方存活征服 / 防守方存活 / 双方存活）
- [ ] 战后清理（cleanDead + 战场控制权更新）
...
### 数据结构
- [ ] CardData ScriptableObject（id/name/region/type/cost/atk/hp/keywords/text/effect/schType/schCost）
...
### 符文系统
- [ ] 6种符文类型定义（blazing/radiant/verdant/crushing/chaos/order）
- [ ] 卡莎虚空符文牌堆（炽烈×7 + 灵光×5 = 12张）
- [ ] 易大师伊欧尼亚符文牌堆（翠意×6 + 摧破×6 = 12张）
- [ ] 符文横置获得法力（tap → G.pMana++）
